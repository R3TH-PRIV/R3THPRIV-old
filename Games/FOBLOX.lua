...
local ChangeNoFallDamage = false
local ChangeGodMode = false
...
Gun:addDropdown("Select Gun", {"Railgun", "Crossbow", "GrenadeLauncher", "RPG", "RainbowRPG", "RayGun", "AK47", "Famas", "Galil", "HoneyBadger", "M249", "NEGEV", "RainbowM16", "SCAR", "MAC10", "MP5", "P90", "PP_Bizon", "TommyGun", "UMP45S", "AWP", "BarrettM82", "MK20", "SSG08", "ArsenalPumpShotgun", "LaserShotGun", "XM1014", "ColtPython", "Deagle", "M1911", "PinkHyperLaser", "USPS"}, function(Value)
    ReplicatedStorage.Events.Weapon.PickUpGunRE:FireServer(Value)
end)

Gun:addToggle("Infinite Ammo", false, function(Value)
    ChangeInfiniteAmmo = Value
    while ChangeInfiniteAmmo and task.wait() do
        if Tool then
            ReplicatedStorage.WeaponsSystem.Network.WeaponReloadRequest:FireServer(Character[Tool.Name])
        end
    end
end)
