--[[
    R3TH PRIV TOWER OF HELL SOURCE
...
print("[ R3TH PRIV ]: R3TH PRIV Tower of Hell loading...")
...
if game.PlaceId == 3582763398 then
    ChangeAutoFarmDelay = 30
else
    ChangeAutoFarmDelay = 15
end
...
tweenService, tweenInfo = game:GetService("TweenService"), TweenInfo.new(ChangeAutoFarmDelay, Enum.EasingStyle.Linear)
    ...
    if type(v) == "function" then
        ...
        if info.name == "kick" then
            if (hookfunction(info.func, function(...)end)) then
                sendnotification("Successfully bypassed kick.", false)
            else
                sendnotification("Failed to bypass kick.", false)
            end
        end
    end
end
...
--------------------------------------------------------------------------------------MAIN----------------------------------------------------------------------------------------
Main:addToggle("Auto Farm", false, function(Value)
    ChangeAutoFarm = Value
    while ChangeAutoFarm do
        GodMode()
        HumanoidRootPart.Anchored = true
        tweenService:Create(HumanoidRootPart, tweenInfo, {CFrame = CFrame.new(Workspace.tower.finishes.Finish.Position)}):Play()
        task.wait(ChangeAutoFarmDelay)
        HumanoidRootPart.Anchored = false
    end
    Humanoid.Health = 0
end)

Main:addToggle("Godmode", false, function(Value)
    ChangeGodMode = Value
    while ChangeGodMode do
        GodMode()
        wait(5)
    end
end)

Main:addButton("Get All Tools", function()
    for _,v in pairs(ReplicatedStorage.Gear:GetDescendants()) do
        if v:IsA("Tool") then
