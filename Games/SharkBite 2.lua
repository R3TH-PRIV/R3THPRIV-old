...
Main:addToggle("Safe AFK Farm", false, function(Value)
    ...
    while ChangeSafeAFKFarm and task.wait() do
        function ChangeSafeAFKFarmFix()
            SitCheck()
            HumanoidRootPart.CFrame = CFrame.new(-123, 95, -119)
            local button = LocalPlayer.PlayerGui.AFK.Page.YesButton
        ...
        pcall(ChangeSafeAFKFarmFix)
    end
end)

Main:addToggle("Sit Check", false, function(Value)
    ChangeSitCheck = Value
end)

Main:addToggle("Invite All", false, function(Value)
    ChangeInviteAll = Value
    while ChangeInviteAll and task.wait() do
        function ChangeInviteAllFix()
            for i,v in pairs (Players:GetChildren()) do
                ReplicatedStorage.EventsFolder.BoatSelection.InviteGuestRF:InvokeServer(v.Name)
            end
        end
        pcall(ChangeInviteAllFix)
    end
end)

Main:addButton("Remove Lobby Barriers", function()
