...
local Main = R3TH:addPage("Main", 5012544944)
...
--------------------------------------------------------------------------------------MAIN----------------------------------------------------------------------------------------
Misc:addDropdown("Select Boat", {"SmallWoodenSailboat", "DuckyBoat", "FishingBoat", "UnicornBoat", "BlueWoodenMotorboat", "DoubleDoughnutTubeBoat", "VikingLongship", "SmallDinghyMotorboat", "HoverBike", "Sloop", "TourBoat", "TugBoat", "CanopyMotorboat", "SharkCageBoat", "Catamaran", "Duckmarine", "Lifeboat", "ViperSpeedBoat", "JetSki", "PartyBoat", "Marlin", "PyroTank", "CombatBoat", "MilitarySubmarine", "Wildfire", "CruiseShip", "HydroTank", "Sleigh2022", "GingerbreadSteamBoat", "Snowmobile", "TheGoldenDucky"}, function(Boat)
    game:GetService("ReplicatedStorage").EventsFolder.BoatSelection.UpdateHostBoat:FireServer(Boat)
    wait()
end)

Misc:addToggle("Safe AFK Farm", false, function(autofarm)
    safeafkfarmloop = autofarm
    while safeafkfarmloop do
        function safeafkfarmloopfix()
        sitcheck()
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-123, 95, -119)
        wait()
    ...
    wait()
    pcall(safeafkfarmloopfix)
    end
end)

Misc:addToggle("Collect All Vases", false, function(collectallvases)
    if collectallvases == true then
        collectallvasesloop = true
        while collectallvasesloop do
            function collectallvasesloopfix()
            sitcheck()
            for i,v in pairs (workspace.Vases:GetDescendants()) do
                if v.Vase.Name == "Vase" then
                    fastcollect = true
                    while fastcollect do
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(v.Vase.Position)
                        wait()
                    end
                    task.wait()
        ...
        wait()
        pcall(collectallvasesloopfix)
        end
    end
    if collectallvases == false then
        collectallvasesloop = false
        fastcollect = false
        wait()
    end
end)

Misc:addToggle("Sit Check", false, function(sitchecktoggle)
    if sitchecktoggle == true then
        sitcheckenabled = true
        wait()
    end
    if sitchecktoggle == false then
        sitcheckenabled = false
        wait()
    end
end)

Misc:addToggle("Instant Kill Shark", false, function(instantkillshark)
    if instantkillshark == true then
        if killsharknotfi ~= true then
            R3TH:Notify("R3TH PRIV", "Hold your gun before turning on and you need to be close to the shark for it to work, will be changed in the future.")
            killsharknotfi = true
        end
        wait()
        killsharkloop = true
        while killsharkloop do
            remote:FireServer(WaitForChildWhichIsA(workspace:WaitForChild("Sharks"),"Model"))
            wait()
        end
    end
    if instantkillshark == false then
        killsharkloop = false
        wait()
    end
end)

Misc:addToggle("Invite All", false, function(inviteall)
    if inviteall == true then
        inviteallloop = true
        while inviteallloop do
            for i,v in pairs (game.Players:GetChildren()) do
                function inviteallfix()
                game:GetService("ReplicatedStorage").EventsFolder.BoatSelection.InviteGuestRF:InvokeServer(v.Name)
            end
            wait()
            pcall(inviteallfix)
            end
    ...
    if inviteall == false then
        inviteallloop = false
        wait()
    end
end)

Misc:addButton("Remove Lobby Barriers", function()
    for i,v in pairs (workspace:GetDescendants()) do
        if v.Name == "LobbyBarriers" then
            v:Destroy()
        end
    end
    task.wait()
