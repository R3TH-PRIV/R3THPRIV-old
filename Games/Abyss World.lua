...
local CheckPoints = game:GetService("Workspace").Map.Ignore.Checkpoints
...
LocalPlayer.CharacterAdded:Connect(function(newCharacter)
    ...
    if ChangeAntiLifeRemove then
        Humanoid.Died:Connect(function()
            if ChangeAntiLifeRemove then
                ReplicatedStorage.Services.RequestRespawn:FireServer()
            end
        end)
    end
end)
...
--------------------------------------------------------------------------------------MAIN----------------------------------------------------------------------------------------
Main:addToggle("Loop Reset", false, function(Value)
    ChangeLoopReset = Value
    while ChangeLoopReset do
        Humanoid.Health = 0
        task.wait(1)
        ReplicatedStorage.Services.RequestRespawn:FireServer()
        task.wait(3)
...
Main:addToggle("Collect All Chests", false, function(Value)
    ChangeCollectAllChests = Value
    while ChangeCollectAllChests and task.wait() do
        for i,v in pairs (Workspace.Map.Main.Props:GetDescendants()) do
            if ChangeCollectAllChests then
                if v.Name == ("Common Chest" or "Rare Chest") then
                    if v:FindFirstChild("Region") ~= nil then
                        HumanoidRootPart.CFrame = CFrame.new(v.Region.Position)
                        wait(1.5)
                    end
                end
            end
...
Main:addToggle("Infinite Lifes", false, function(Value)
    ChangeAntiLifeRemove = Value
    Humanoid.Died:Connect(function()
        if ChangeAntiLifeRemove then
            ReplicatedStorage.Services.RequestRespawn:FireServer()
        end
...
Main:addButton("Get All Checkpoints", function()
    for _, v in ipairs(PositionTable) do
       if not LocalPlayer.Checkpoints:FindFirstChild(v[2].Name) then
           HumanoidRootPart.CFrame = CFrame.new(v[1])
           wait(1.5)
           while not LocalPlayer.Checkpoints:FindFirstChild(v[2].Name) do
               wait()
               fireproximityprompt(v[2].Part.ProximityPrompt, (HumanoidRootPart.Position - v[1]).Magnitude)
               HumanoidRootPart.CFrame = CFrame.new(v[1])
           end
       end
    end
...
Main:addDropdown("Teleport To Checkpoint", {"Checkpoint 0: Temple of the Abyss", "Checkpoint 1: Windmill Town", "Checkpoint 2: Moss Underhang", "Checkpoint 3: Worker's Burrow", "Checkpoint 4: Wooded Boundary", "Checkpoint 5: Ruined Umbra", "Checkpoint 6: Root Canal", "Checkpoint 7: Cardinal Spire", "Checkpoint 8: Stagnant Pipeworks", "Checkpoint 9: Sprouting Baths", "Checkpoint 10: Diminished Spire", "Checkpoint 11: Plantasia"}, function(Value)
    for _, v in ipairs(PositionTable) do
        if v[2].Name == Value then
            HumanoidRootPart.CFrame = CFrame.new(v[1])
        end
    end
end)

Main:addDropdown("Teleport to Player", playerlist, function(Value)
    TPTarget = Players:FindFirstChild(Value)
    HumanoidRootPart.CFrame = CFrame.new(TPTarget.Character:WaitForChild("HumanoidRootPart").Position)
end)
