...
local teleportLocations = {
    ...
    ["Armour Shop"] = CFrame.new(-1359, 13, 161),
    ["Backpack Shop"] = CFrame.new(-1350, 13, -50),
    ["Bank"] = CFrame.new(-1372, 13, -138),
    ["Gun Room"] = CFrame.new(-1060, -9, -189),
    ["Boat Room"] = CFrame.new(-996, -12, -209),
    ["Vacation Island"] = CFrame.new(331, 53, 12),
    ["Spawn"] = CFrame.new(-1260, 13, 17),
    ["Army Base"] = CFrame.new(-560, 2, 4137),
    ["Out Of Map"] = CFrame.new(-2604, 12, -636),
    ["Crystal Room"] = CFrame.new(-1193, -174, 55),
    ["Sewers"] = CFrame.new(-1249, -8, 100),
    ["Sell Shop"] = CFrame.new(-1358, 13, 9),
    ["V.I.P Room"] = CFrame.new(-1189, -9, -158),
    ["Donut Room"] = CFrame.new(-1115, -9, -192),
    ["Gold Room"] = CFrame.new(-1157, -9, -175),
    ["Secret Room"] = CFrame.new(-1199, -9, -21)
}
...
Main:addButton("Open Safe", function()
    ReplicatedStorage.Events.Safe.RequestSafe:FireServer()
end)

Main:addButton("Loot Dynamite", function()
    ReplicatedStorage.Events.Loot.RequestLoot:FireServer(math.random(489, 493))
end)

Main:addButton("Sell All", function()
    ReplicatedStorage.Events.Shop.RequestSellAll:FireServer(-1)
end)

Main:addButton("Unlock Level Access", function()
    LevelAccess = 0
    for i = 1, 7 do
        ReplicatedStorage.Events.Access.RequestBuyAccess:FireServer(LevelAccess)
        LevelAccess = LevelAccess + 1
    end
end)

Main:addToggle("Explode All Cars", false, function(Value)
    ChangeExplodeAllCars = Value
    while ChangeExplodeAllCars and task.wait(0.1) do
...
Main:addToggle("Kill All Police", false, function(Value)
    ...
    while ChangeKillAllPolice and task.wait(0.1) do
...
Main:addToggle("Kill All NPCs", false, function(Value)
    ...
    while ChangeKillAllNPCs and task.wait(0.1) do
...
Main:addTextbox("Request Loot Level", "0 - 500", function(Value, focusLost)
    ...
    if focusLost then
        if LootLevel > -1 and LootLevel < 501 then
            ReplicatedStorage.Events.Loot.RequestLoot:FireServer(LootLevel)
        else
            sendnotification("For this to work, your loot level should be between 0 and 500.", true)
        end
    end
end)
...
Farm:addToggle("Start Money Farm", false, function(Value)
...
Farm:addToggle("XP Farm", false, function(Value)
    ChangeXPFarm = Value
...
Target00:addDropdown("Select Player", playerlist, function(Value)
    ChangeGameTarget = Value
end)
...
--------------------------------------------------------------------------------------SHOP----------------------------------------------------------------------------------------
Guns:addDropdown("Select Gun", {"Glock17", "1911", "TEC9", "Shotgun", "UZI", "AK47", "M4A1", "Golden1911", "GoldenShotgun", "GoldenUZI", "GoldenM4A1", "RareUZI", "RareM4A1"}, function(Value)
    ChangeSelectGun = Value
end)

Guns:addButton("Buy Gun", function()
    ReplicatedStorage.Events.GunShop.RequestBuy:FireServer(ChangeSelectGun)
end)

Guns:addToggle("Loop Buy Gun", false, function(Value)
    ChangeLoopBuyGun = Value
    while ChangeLoopBuyGun and task.wait() do
        ReplicatedStorage.Events.GunShop.RequestBuy:FireServer(ChangeSelectGun)
    end
end)

Armour:addDropdown("Select Armour", {"LightBulletProofVest1", "LightBulletProofVest2", "LightBulletProofVest3", "HeavyBulletProofVest1", "HeavyBulletProofVest2", "HeavyBulletProofVest3", "HeavyBodyArmour1", "HeavyBodyArmour2", "HeavyBodyArmour3"}, function(Value)
    ChangeSelectArmour = Value
end)

Armour:addButton("Buy Armour", function()
    ReplicatedStorage.Events.ArmourShop.RequestBuy:FireServer(ChangeSelectArmour)
end)

Armour:addToggle("Loop Buy Armour", false, function(Value)
    ChangeLoopBuyArmour = Value
    while ChangeLoopBuyArmour and task.wait() do
        ReplicatedStorage.Events.ArmourShop.RequestBuy:FireServer(ChangeSelectArmour)
    end
end)

Pets:addDropdown("Select Pet", {"Husky", "OrangeCat", "Chihuahua", "GreyCat", "Cattledog", "Jackrussel", "Siamese", "Bulldog"}, function(Value)
    ReplicatedStorage.Events.PetShop.RequestBuy:FireServer(Value)
    task.wait()
    ReplicatedStorage.Events.PetShop.RequestEquip:FireServer(Value)
end)
...
Tools:addButton("Best Safe Tool", function()
    ReplicatedStorage.Events.Toolshop.RequestBuy:FireServer("Stethoscope", "Safetools")
end)

Tools:addButton("Best Electrical Tool", function()
    ReplicatedStorage.Events.Toolshop.RequestBuy:FireServer("Hackingdevice", "Electricaltools")
end)

Tools:addButton("Best Picklock", function()
    ReplicatedStorage.Events.Toolshop.RequestBuy:FireServer("Automaticpicklock", "Picklocks")
end)

Tools:addButton("Best Normal Tool", function()
    ReplicatedStorage.Events.Toolshop.RequestBuy:FireServer("Plasmacutter", "Tools")
end)
